...
function debugWithTimer(set, cb) {
  ...
  /**
   * Cheap probe for whether the timers would do anything if invoked.
   * Allows callers to skip constructing timer labels entirely on hot
   * paths when both NODE_DEBUG logging and tracing are disabled.
   * Reads the live trace category buffer so that tracing enabled at
   * runtime is still picked up.
   * @returns {boolean}
   */
  function timersEnabled() {
    ...
    return debugLogCategoryEnabled || traceCategoryBuffer[0] !== 0;
  }
